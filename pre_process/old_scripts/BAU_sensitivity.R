...
res <- read_csv("baseline_sens_res.csv")
filt <- filter(res, run == 1 & cpu > -200)

ggplot(filt, aes(relative_carb, cpu), group = factor(run)) +
  geom_line()
...
test <- bind_rows(cum_res)

single <- test %>% 
  filter(run == 1)

ggplot(single, aes(cumsum_carb, cpu), group = factor(run), color = factor(run)) +
  geom_line() +
...
p <- ggplot(cumsum, aes(cumsum_carb, cpu)) +
  geom_line(color = 10) 
...
for (i in 2:10){
  ...
  p <- p + geom_line(cumsum, mapping = aes(cumsum_carb, cpu), color = i)
}
p +
